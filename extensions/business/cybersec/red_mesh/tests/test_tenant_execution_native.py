...
def _render_native(default_route=None):
  ...
  for original in plugin.body:
    if not isinstance(original, ast.FunctionDef) or original.name not in ROUTES + READ_ROUTES + EFFECT_ROUTES + UNGUARDED_ROUTES:
      continue
  ...
  for name in ROUTES + READ_ROUTES + EFFECT_ROUTES + UNGUARDED_ROUTES:
