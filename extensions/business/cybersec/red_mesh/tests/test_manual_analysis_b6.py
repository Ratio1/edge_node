...
RM-084 P3 removed the unscoped half: the caller names a tenant, the gate is the matrix's own
`analysis:run`, and that operation binds Allow Pentester -- so every case here says which side of
that switch it is on.
"""
import asyncio
import json
...
@pytest.mark.parametrize("fault,status", (
  ...
  ("tenant_user", 403), ("pentesting_off", 403), ("missing_tenant", 400),
  ("unknown_tenant", 404), ("identity_store", 503),
))
...
def test_the_denial_keeps_its_status_over_the_wire(read_native, response_format, fault, status):
  ...
  with read_endpoint_fixture(bound=True, role="tenant_pentester") as fixture:
    ...
    payload = {**BODY, "request_actor": fixture.actor, "tenant_id": allow_pentester(fixture)}
...
def test_the_typed_operational_codes_still_reach_the_wire(read_native, response_format):
  ...
  with read_endpoint_fixture(bound=True, role="tenant_pentester") as fixture:
    ...
    status, _headers, body, calls = asyncio.run(request(module, "analyze_job",
      {**BODY, "request_actor": fixture.actor, "tenant_id": tenant_id}))
    assert calls == 1
