...
- Each coverage_gaps or out_of_scope item must name a concrete fact from the
  context: a probe, a port, a protocol or a scope entry. An item that names none
  is filler; use [] instead.
...
- coverage_gaps: zero to three limitations of this scan against this target: checks a
...
- coverage_gaps: zero to three limitations of this scan itself or follow-up tests.
- Each coverage_gaps or out_of_scope item must name a concrete fact from the
  context: a probe, a port, a protocol or a scope entry. An item that names none
  is filler; use [] instead.
...
LOCAL_REPORT_CHUNKS = (
  ...
  LocalReportChunk(
    ...
    prompt=(
      ...
      "strings grounded only in provided findings. coverage_gaps is zero to three "
      ...
      "across both. Each coverage_gaps or out_of_scope item must name a concrete "
      "fact from the context: a probe, a port, a protocol or a scope entry. An "
      "item that names none is filler; use [] instead. conclusion is exactly two "
      "complete sentences with the next "
      "operating decision."
    ),
  ),
)
