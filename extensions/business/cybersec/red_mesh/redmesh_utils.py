...
class PentestLocalWorker(
  ...
  def get_status(self, for_aggregations=False):    
    ...
    dct_status["scan_metrics"] = self.metrics.build().to_dict()
  ...
  def _interruptible_sleep(self):
    ...
    self.metrics.record_port_scan_delay(delay)
    time.sleep(delay)
  ...
  def execute_job(self):
    ...
    try:
      ...
      self.metrics.start_scan(len(self.initial_ports))

      if not self._check_stopped():
        self.metrics.phase_start("port_scan")
        self._scan_ports_step()
        self.metrics.phase_end("port_scan")

      if not self._check_stopped():
        self.metrics.phase_start("fingerprint")
        self._active_fingerprint_ports()
        self.metrics.phase_end("fingerprint")
        self.state["completed_tests"].append("fingerprint_completed")

      if not self._check_stopped():
        self.metrics.phase_start("service_probes")
        self._gather_service_info()
        self.metrics.phase_end("service_probes")
        self.state["completed_tests"].append("service_info_completed")

      if not self._check_stopped() and not self._ics_detected:
        self.metrics.phase_start("web_tests")
        self._run_web_tests()
        self.metrics.phase_end("web_tests")
        self.state["completed_tests"].append("web_tests_completed")

      if not self._check_stopped():
        self.metrics.phase_start("correlation")
        self._post_scan_correlate()
        self.metrics.phase_end("correlation")
  ...
  def _scan_ports_step(self, batch_size=None, batch_nr=1):
    ...
    for i, port in enumerate(ports_batch):
      ...
      try:
        ...
        if result == 0:
          self.metrics.record_connection("connected", conn_time)
          ...
          if result == errno.ETIMEDOUT:
            self.metrics.record_connection("timeout", conn_time)
          elif result == errno.ECONNREFUSED:
            self.metrics.record_connection("refused", conn_time)
          elif result == errno.ECONNRESET:
            self.metrics.record_connection("reset", conn_time)
          else:
            self.metrics.record_connection("refused", conn_time)
      except Exception as e:
        self.metrics.record_connection("error", time.time() - t0)
  ...
  def _gather_service_info(self):
    ...
    for method in service_info_methods:
      ...
      self.metrics.record_probe(method, "completed")
    # end for each method
      ...
      for port in ports_to_test:
        ...
        if iter_result is not None:
          ...
          if isinstance(iter_result, dict):
            for f in iter_result.get("findings", []):
              sev = f.get("severity", "INFO") if isinstance(f, dict) else "INFO"
              self.metrics.record_finding(sev)
