...
@dataclass(frozen=True)
class LauncherLiveness:
  """
  Launcher-owned liveness for one job, published on a heartbeat.

  Stored in its own CStore hset (hkey = f"{instance_id}:live:launcher", key = job_id) rather than
  in the job record. Two reasons, both load-bearing:

  * ``models/cstore.py`` already states that runtime liveness belongs in the ``:live`` namespace,
    and the job record keeps durable orchestration metadata. A 30-second heartbeat in the job record
    would be 120 full-record rewrites and 120 ``job_revision`` bumps on a one-hour job, starving the
    ``reject_stale=True`` archive-prune writers.
  * ``_write_job_record`` refuses any write where ``current["launcher"]`` is not this node, so a
    successor could not record its own takeover if ownership liveness lived there. D1 needs it to.

  A separate hset rather than a key inside ``:live`` so a launcher row cannot collide with a worker
  row (``{job_id}:{worker_addr}``) by construction.

  ``launcher`` is on the row because a heartbeat is evidence about its writer, not about the job:
  without it a stale successor's row would read as the current owner being alive.
  """
  job_id: str
  launcher: str
  launcher_since: float
  last_seen_at: float
  ...
  @classmethod
  def from_dict(cls, d: dict) -> "LauncherLiveness":
    if not isinstance(d, dict):
      raise TypeError("Invalid launcher liveness payload")
    job_id, launcher = d["job_id"], d["launcher"]
...
def launcher_liveness_state(row, *, now: float, loss_after: float) -> str:
  """Classify one launcher's liveness. Four states, and the distinctions matter.

  ``unavailable`` -- no row at all. Never heartbeated is not the same statement as heartbeated and
  aged out, and C2 must not act on the first as though it were a loss.
  ``unresolved`` -- the timestamp is in the future or not a finite number. The parent design says
  invalid or future timing is unresolved; treating a skewed clock as live would let a node hold
  ownership forever, and treating it as lost would hand the job away on a skew.
  ``lost`` / ``live`` -- an actual measurement against the threshold.
  """
  if row is None:
    return "unavailable"
  last_seen = getattr(row, "last_seen_at", None)
  if not isinstance(last_seen, (int, float)) or last_seen != last_seen or last_seen in (
      float("inf"), float("-inf")) or last_seen < 0:
    return "unresolved"
  age = now - last_seen
  if age < 0:
    return "unresolved"
  return "lost" if age >= loss_after else "live"
