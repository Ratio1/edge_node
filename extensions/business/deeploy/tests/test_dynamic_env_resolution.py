...
class DeeployDynamicEnvResolutionTests(unittest.TestCase):
  ...
  def test_resolve_shmem_in_plugins_canonicalizes_whitespace_padded_type(self):
    plugin = make_deeploy_plugin()
    plugins = [
      {
    ...
    entry = consumer_instance["DYNAMIC_ENV"]["API_HOST"][0]
    self.assertEqual(entry["type"], "shmem")
    self.assertEqual(entry["path"], ["app-123__provider", "CONTAINER_IP"])
    self.assertEqual(consumer_instance["SEMAPHORED_KEYS"], ["app-123__provider"])

  def test_validate_dynamic_env_config_rejects_unsupported_backend_types(self):
    plugin = make_deeploy_plugin()
    for entry_type in ("container_ip", "plugin_value", "SHMEM"):
      plugins = [
        {
          plugin.ct.CONFIG_PLUGIN.K_SIGNATURE: "CONTAINER_APP_RUNNER",
          plugin.ct.CONFIG_PLUGIN.K_INSTANCES: [
      ...
      with self.subTest(entry_type=entry_type):
        with self.assertRaisesRegex(ValueError, "unsupported type '{}'".format(entry_type)):
          plugin._validate_dynamic_env_config(plugins)

  def test_validate_dynamic_env_config_rejects_empty_or_non_string_type(self):
    plugin = make_deeploy_plugin()
    for entry_type in ("", "   ", None, 123):
      plugins = [
        {
          plugin.ct.CONFIG_PLUGIN.K_SIGNATURE: "CONTAINER_APP_RUNNER",
          plugin.ct.CONFIG_PLUGIN.K_INSTANCES: [
      ...
      with self.subTest(entry_type=entry_type):
        with self.assertRaisesRegex(ValueError, "must include a non-empty type"):
          plugin._validate_dynamic_env_config(plugins)
