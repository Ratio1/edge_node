...
- ID: `ML-20260521-001`
- Timestamp: `2026-05-21T17:04:30Z`
- Type: `change`
- Summary: Deeploy request authentication now supports human-readable wrapper signing over a compact canonical payload hash.
- Criticality: Cross-repo security contract change affecting wallet-signature UX and backend authorization across `deeploy-dapp`, `edge_node`, `naeural_core`, and `ratio1_sdk`.
- Details: Deeploy v3 requests carry `EE_HASH` for the compact canonical payload hash while `EE_ETH_SIGN` signs a human-readable wrapper containing `Request type`, optional plugin/node counts, and the hash. Backend verification keeps legacy v2 dual-accept behavior when `EE_HASH` is absent, but v3 verification rejects hash/signature mismatches and verifies the wrapper text through the exposed EVM text-signature path. Request-type labels are centralized constants on both frontend and backend.
- Verification: `PYTHONPATH=naeural_core python -m unittest discover -s extensions/business/deeploy/tests -p 'test_*.py'`; `python -m unittest tests.test_compact_canonical_json`; `python -m unittest tests.test_evm_safe_signature`; `python -m compileall naeural_core/utils/plugins_base/bc_wrapper.py`; `npm run test:deeploy-message`
- Links: `extensions/business/deeploy/deeploy_mixin.py`, `extensions/business/deeploy/deeploy_manager_api.py`, `extensions/business/deeploy/deeploy_const.py`, `extensions/business/deeploy/tests/test_deeploy_v3_wrapper.py`, `naeural_core/utils/plugins_base/bc_wrapper.py`, `ratio1_sdk/ratio1/bc/base.py`, `deeploy-dapp/src/lib/deeploy-utils.ts`
