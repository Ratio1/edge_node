...
- ID: `ML-20260611-001`
- Timestamp: `2026-06-11T08:29:40Z`
- Type: `change`
- Summary: Container-app runtime images now preflight the Docker SDK import required by CAR/WAR.
- Criticality: Operational reliability change for stack and worker app deployment; a missing Docker SDK prevents `CONTAINER_APP_RUNNER` and `WORKER_APP_RUNNER` from loading.
- Details: The repo has a top-level `docker/` directory, so when the Python Docker SDK is absent the import can resolve as an empty namespace package and fail later as `No module named 'docker.types'`. Runtime and local requirements now declare `docker>=7.1.0`, devcontainer requirements include the same SDK, Docker image builds preflight `from docker.types import DeviceRequest`, and CAR raises an explicit missing-SDK message.
- Verification: `python3 -m unittest tests.test_comms_testbed_config`; `python3 -m py_compile device.py constants.py`; `python3 -m unittest extensions.business.container_apps.test_worker_app_runner`; `docker build -t local_edge_node -f Dockerfile_devnet .`; `docker run --rm --entrypoint python3 local_edge_node -c "from docker.types import DeviceRequest; print(DeviceRequest.__name__)"`
- Links: `requirements.txt`, `requirements_local.txt`, `.devcontainer/requirements.txt`, `Dockerfile_devnet`, `Dockerfile_testnet`, `Dockerfile_mainnet`, `Dockerfile_devnet_local`, `.devcontainer/Dockerfile`, `extensions/business/container_apps/container_app_runner.py`
