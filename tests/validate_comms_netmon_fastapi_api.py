...
def _validate_probe(observer_eeid, target_eeid, probe, failures):
  ...
  if summary_expected:
    ...
    _assert(
      _value(probe, "is_online_for_control") is True,
      f"{observer_eeid}->{target_eeid}: control online predicate is not true",
      failures,
    )
